...
var name_selectMathTschirnhaus    = "selectMathTschirnhaus";
...
var name_inputFieldEliminatePowers = "inputFieldEliminatePowers";
...
function initJavascript() {
  ...
  elem_selectMathTschirnhaus     = document.getElementById( name_selectMathTschirnhaus     );
  ...
  elem_inputFieldEliminatePowers = document.getElementById( name_inputFieldEliminatePowers );
  ...
  elem_selectMathTschirnhaus.addEventListener(  "change", fncSelectMathChange    );
  ...
  fieldInputArray.push( elem_selectMathTschirnhaus  );
  ...
  fieldInputArray.push( elem_inputFieldEliminatePowers );
...
function fieldDeleteFromSelect( key ) {
   for ( iCount = 0; iCount < elem_selectMathTschirnhaus.length; iCount++ ) {
     if (elem_selectMathTschirnhaus.options[ iCount ].value == key )
        elem_selectMathTschirnhaus.remove( iCount );
   }
}
...
function fncFieldsUpdate( dataJson ) {
   if ( 'name'  in dataJson ) {
      ...
      var currentValue = elem_selectMathTschirnhaus.value;
      if ( currentValue != dataJson.name ) {
        var option = document.createElement("option");
        option.text = dataJson.name;
        elem_selectMathTschirnhaus.add( option );
        elem_selectMathTschirnhaus.value = dataJson.name;
      }
   ...
   if ( 'eliminatePowers'  in dataJson ) {
      elem_inputFieldEliminatePowers.value  = dataJson.eliminatePowers;
   } else {
      elem_inputFieldEliminatePowers.value  = "";
   }
...
function fncFieldGet() {
   ...
   data.eliminatePowers = fncInputGetNumber( name_inputFieldEliminatePowers, "Eliminate" ) ;
...
async function asyncDownloadInputData(key) {
  try {
     ...
     url = '?prog=tschirnhausJsonData&key=' + key ;
...
async function asyncSaveInputData( options = "" ) {
  try {
     ...
     url = '?prog=tschirnhausJsonSave&options=' + options  ;
...
async function asyncDeleteInputData(key) {
  try {
     ...
     url = '?prog=tschirnhausJsonDelete&key=' + key ;
...
function fncSelectMathChange() {
  asyncDownloadInputData( elem_selectMathTschirnhaus.value );
}
