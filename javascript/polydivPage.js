...
var name_selectMathPolyDiv    = "selectMathPolyDiv";
...
var elem_selectMathPolyDiv    = null;
...
function initJavascript() {
  ...
  elem_selectMathPolyDiv   = document.getElementById( name_selectMathPolyDiv   );
  ...
  elem_selectMathPolyDiv.addEventListener(    "change", fncSelectMathChange    );
  ...
  fieldInputArray.push( elem_selectMathPolyDiv  );
...
function fieldDeleteFromSelect( key ) {
   for ( iCount = 0; iCount < elem_selectMathPolyDiv.length; iCount++ ) {
     if (elem_selectMathPolyDiv.options[ iCount ].value == key )
        elem_selectMathPolyDiv.remove( iCount );
   }
}
...
function fncFieldsUpdate( dataJson ) {
   if ( 'name'  in dataJson ) {
      ...
      var currentValue = elem_selectMathPolyDiv.value;
      if ( currentValue != dataJson.name ) {
        var option = document.createElement("option");
        option.text = dataJson.name;
        elem_selectMathPolyDiv.add( option );
        elem_selectMathPolyDiv.value = dataJson.name;
      }
...
async function asyncDownloadInputData(key) {
  try {
     ...
     url = '?prog=polydivJsonData&key=' + key ;
...
async function asyncSaveInputData( options = "" ) {
  try {
     ...
     url = '?prog=polydivJsonSave&options=' + options  ;
...
async function asyncDeleteInputData(key) {
  try {
     ...
     url = '?prog=polydivJsonDelete&key=' + key ;
...
function fncSelectMathChange() {
  asyncDownloadInputData( elem_selectMathPolyDiv.value );
}
