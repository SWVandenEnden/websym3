...
var name_selectMathFourthpower  = "selectMathFourthpower";
...
var name_inputFieldE            = "inputFieldE" ;
...
var elem_selectMathFourthpower  = null;
...
function initJavascript() {
  ...
  elem_selectMathFourthpower  = document.getElementById( name_selectMathFourthpower );
  ...
  elem_inputFieldE           = document.getElementById( name_inputFieldE            );
  ...
  elem_selectMathFourthpower.addEventListener("change", fncSelectMathChange    );
  ...
  fieldInputArray.push( elem_selectMathFourthpower );
  ...
  fieldInputArray.push( elem_inputFieldE          );
...
function fieldDeleteFromSelect( key ) {
   for ( iCount = 0; iCount < elem_selectMathFourthpower.length; iCount++ ) {
     if (elem_selectMathFourthpower.options[ iCount ].value == key )
        elem_selectMathFourthpower.remove( iCount );
   }
}
...
function fncFieldsUpdate( dataJson ) {
   if ( 'name' in dataJson ) {
      ...
      var currentValue = elem_selectMathFourthpower.value;
      if ( currentValue != dataJson.name ) {
        var option = document.createElement("option");
        option.text = dataJson.name;
        elem_selectMathFourthpower.add( option );
        elem_selectMathFourthpower.value = dataJson.name;
      }
   ...
   if ( 'e' in dataJson ) {
      elem_inputFieldE.value = dataJson.e;
   } else {
      elem_inputFieldE.value = "";
   }
...
async function asyncDownloadInputData(key) {
  try {
     ...
     url = '?prog=fourthpowerJsonData&key=' + key ;
...
async function asyncSaveInputData( options = "" ) {
  try {
     ...
     url = '?prog=fourthpowerJsonSave&options=' + options  ;
...
async function asyncDeleteInputData(key) {
  try {
     ...
     url = '?prog=fourthpowerJsonDelete&key=' + key ;
...
function fncSelectMathChange() {
  asyncDownloadInputData( elem_selectMathFourthpower.value );
}
