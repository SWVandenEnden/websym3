...
var name_selectMathTaylorserie  = "selectMathTaylorSerie";

var name_inputFieldCode         = "inputFieldCode"      ;
var name_inputFieldFormula      = "inputFieldFormula"   ;
var name_inputFieldSteps        = "inputFieldSteps"     ;
var name_inputFieldBaseValue    = "inputFieldBaseValue" ;
var name_inputFieldDiffVar      = "inputFieldDiffVar"   ;
...
var elem_selectMathTaylorserie  = null;

var elem_inputFieldCode         = null;
var elem_inputFieldFormula      = null;
var elem_inputFieldSteps        = null;
var elem_inputFieldBaseValue    = null;
var elem_inputFieldDiffVar      = null;
...
function initJavascript() {
  ...
  elem_selectMathTaylorserie  = document.getElementById( name_selectMathTaylorserie   );
  ...
  elem_inputFieldSteps       = document.getElementById( name_inputFieldSteps        );
  elem_inputFieldBaseValue   = document.getElementById( name_inputFieldBaseValue    );
  elem_inputFieldDiffVar     = document.getElementById( name_inputFieldDiffVar      );
  ...
  elem_selectMathTaylorserie.addEventListener("change", fncSelectMathChange    );
  ...
  fieldInputArray.push( elem_selectMathTaylorserie );

  fieldInputArray.push( elem_inputFieldCode       );
  fieldInputArray.push( elem_inputFieldFormula    );
  fieldInputArray.push( elem_inputFieldSteps      );
  fieldInputArray.push( elem_inputFieldBaseValue  );
  fieldInputArray.push( elem_inputFieldDiffVar    );
...
function fieldDeleteFromSelect( key ) {
   for ( iCount = 0; iCount < elem_selectMathTaylorserie.length; iCount++ ) {
     if (elem_selectMathTaylorserie.options[ iCount ].value == key )
        elem_selectMathTaylorserie.remove( iCount );
   }
}
...
function fncFieldsUpdate( dataJson ) {
   if ( 'name'  in dataJson ) {
      ...
      var currentValue = elem_selectMathTaylorserie.value;
      if ( currentValue != dataJson.name ) {
        var option = document.createElement("option");
        option.text = dataJson.name;
        elem_selectMathTaylorserie.add( option );
        elem_selectMathTaylorserie.value = dataJson.name;
      }
   ...
   if ( 'steps'  in dataJson ) {
      elem_inputFieldSteps.value  = dataJson.steps;
   } else {
      elem_inputFieldSteps.value  = "";
   }

   if ( 'baseValue'  in dataJson ) {
      elem_inputFieldBaseValue.value  = dataJson.baseValue;
   } else {
      elem_inputFieldBaseValue.value  = "";
   }

   if ( 'diffVar'  in dataJson ) {
      elem_inputFieldDiffVar.value  = dataJson.diffVar;
   } else {
      elem_inputFieldDiffVar.value  = "";
   }
...
function fncFieldGet() {
   ...
   data.steps      = fncInputGetNumber( name_inputFieldSteps     );
   data.baseValue  = fncInputGetNumber( name_inputFieldBaseValue );
   data.diffVar    = elem_inputFieldDiffVar.value   ;
...
async function asyncDownloadInputData(key) {
  try {
     ...
     url = '?prog=taylorSerieJsonData&key=' + key ;
...
async function asyncSaveInputData( options = "" ) {
  try {
     ...
     url = '?prog=taylorSerieJsonSave&options=' + options  ;
...
async function asyncDeleteInputData(key) {
  try {
     ...
     url = '?prog=taylorSerieJsonDelete&key=' + key ;
...
function fncSelectMathChange() {
  asyncDownloadInputData( elem_selectMathTaylorserie.value );
}
